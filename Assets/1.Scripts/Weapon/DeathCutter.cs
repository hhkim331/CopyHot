...
public class DeathCutter : CutterBehaviour, IPoolObject
{
    ...
    public void Cut(Transform cuttingTarget, Vector3 cutterPosition, Vector3 cutterNormal)
    {
        ...
        {
            base.Cut(target, cutterPosition, cutterNormal, null, OnCreated);
        }
    }
    ...
    public void CutTriple(Transform cuttingTarget, Transform cutterTransform)
    {
        ...
        foreach (var target in targets)
        {
            base.Cut(target, cutterTransform.position, cutterTransform.up, null, Cut2);
        }
    }
    ...
    void Cut2(Info info, MeshCreationData cData)
    {
        ...
        for (int i = 0; i < cLength; i++)
        {
            base.Cut(cData.CreatedTargets[i], tCutterTransform.position, (tCutterTransform.up + tCutterTransform.right).normalized, null, Cut3);
        }
    }
    ...
    void Cut3(Info info, MeshCreationData cData)
    {
        ...
        for (int i = 0; i < cLength; i++)
        {
            base.Cut(cData.CreatedTargets[i], tCutterTransform.position, (tCutterTransform.up - tCutterTransform.right).normalized, null, OnCreated);
        }
    }
